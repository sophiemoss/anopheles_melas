...
# %% Set threshold
bijagos_threshold = 4
cameroon_threshold = -4

# Set up the plot
fig, ax = plt.subplots(figsize=(10, 6))

# Ensure that pos, chrom, and xpehh_raw are all numpy arrays to support advanced indexing
pos = np.array(callset['variants/POS'][:])
chrom = np.array(callset['variants/CHROM'][:])
xpehh_raw = np.array(xpehh_raw)
...
for unique_chrom in np.unique(chrom[chrom != 'Y_unplaced']):
    ...
    chrom_xpehh_values = xpehh_raw[chrom_mask]
...
bijagos_threshold_mask = xpehh_raw >= bijagos_threshold
cameroon_threshold_mask = xpehh_raw <= cameroon_threshold

# %% Apply the mask to filter the data
bij_significant_chrom = chrom[bijagos_threshold_mask]
bij_significant_pos = pos[bijagos_threshold_mask]
bij_significant_xpehh = xpehh_raw[bijagos_threshold_mask]

cam_significant_chrom = chrom[cameroon_threshold_mask]
cam_significant_pos = pos[cameroon_threshold_mask]
cam_significant_xpehh = xpehh_raw[cameroon_threshold_mask]
...
df_significant_bij_xpehh = pd.DataFrame(bij_significant_xpehh_data, columns=['Chromosome', 'Position', 'XP-EHH'])
df_significant_cam_xpehh = pd.DataFrame(cam_significant_xpehh_data, columns = ['Chromosome', 'Position', 'XP-EHH'])

# %% Save to csv
df_significant_bij_xpehh.to_csv('df_significant_bij_xpehh.csv', index=False)
df_significant_cam_xpehh.to_csv('df_significant_cam_xpehh.csv', index=False)