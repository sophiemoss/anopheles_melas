...
for bamfile in bamfiles:
    # Calculate average coverage for Mt and 3R chromosomes
    mt_coverage = calculate_average_coverage(bamfile, 'Mt')
    r3_coverage = calculate_average_coverage(bamfile, '3R')

    # Calculate coverage ratio of Mt to 3R to prevent division by zero
    coverage_ratio = mt_coverage / r3_coverage if r3_coverage > 0 else 0
    ...
    # Append results, including Mt and 3R coverages, their ratio, and the sex call
    results.append([bamfile, mt_coverage, r3_coverage, coverage_ratio, sex])
